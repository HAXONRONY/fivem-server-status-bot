...
exports.start = function(SETUP) {
  ...
  const UpdateEmbed = function() {
    ...
    .setAuthor("AREA 51 Server Status", "https://cdn.discordapp.com/attachments/618019002027081729/692519373373440080/Area_51.png")
    .setColor(0x2894C2)
    .setFooter(TICK_N % 2 === 0 ? '⚪ AREA 51' : '⚫ RG RONY')
    .setTimestamp(new Date())
    .addField('\n\u200b\nServer IP:','103.114.38.154:30120             [SUBSCRIBE](https://www.youtube.com/paglagaming/)\n\u200b\n',false)
    if (STATUS !== undefined)
    {
  ...
  const offline = function() {
    ...
    .addField('Current Queue','?',true)
    .addField('Online Player','?\n\u200b\n',true);
    sendOrUpdate(embed);
    LAST_COUNT = null;
  };
  ...
  const updateMessage = function() {
    getVars().then((vars) => {
      getPlayers().then((players) => {
        ...
        .addField('Current Queue',queue === 'Enabled' || queue === undefined ? '0' : queue.split(':')[1].trim(),true)
        .addField('Online Player',`${players.length}/${MAX_PLAYERS}\n\u200b\n`,true);
        // .addField('\u200b','\u200b\n\u200b\n',true);
        if (players.length > 0) {
          // method D
          const fieldCount = 3;
          ...
          for (var i=0;i<players.length;i++) {
            fields[(i+1)%fieldCount] += `${players[i].name.substr(0,1)}\n`; // first 12 characters of players name
          }
  ...
  bot.on('ready',() => {
    ...
    bot.user.setActivity('AREA 51 RP',{'url':'https://www.twitch.tv/RoqueTV','type':'STREAMING'});
